...
else if ($typeid == 2 && Copyright)
{
	...
	$mid = $_POST['mid'];
	$sql = "SELECT `g_type`, `h1`, `h2`, `h3`, `h4`, `h5`, `h6`, `h7`, `h8`, `h9`, `h10`, `h11`, `h12`, `h13`, `h14` FROM `g_odds8` WHERE g_type = 'Ball_{$mid}' OR g_type ='Ball_6' OR g_type='Ball_7' OR g_type ='Ball_8' OR g_type='Ball_9' ORDER BY g_id ASC";
	...
	for ($i=0; $i<count($oddsResult); $i++)
	{
		foreach ($oddsResult[$i] as $k=>$v)
		{
			...
			if ($oddsResult[$i]['g_type'] == 'Ball_1' || $oddsResult[$i]['g_type'] == 'Ball_2' || $oddsResult[$i]['g_type'] == 'Ball_3' || $oddsResult[$i]['g_type'] == 'Ball_4' || $oddsResult[$i]['g_type'] == 'Ball_5')
			{
				...
				{
					$arrList[$i][$k] = setoddsxj($k, $v, NULL, $user, 0,$oddsResult[$i]['g_type']);
				}
				else if ($k != 'g_type' && $v != null)
				{
					$arrList[$i][$k] = setoddsxj($k, $v, NULL, $user, 1);
				}
			}
			else if ($oddsResult[$i]['g_type'] == 'Ball_6' && $k != 'g_type' && $v != null && Copyright)
			{
				$arrList[$i][$k] = setoddsxj($k, $v, NULL, $user, 1,$oddsResult[$i]['g_type']);
			}
			else if (($oddsResult[$i]['g_type'] == 'Ball_7'||$oddsResult[$i]['g_type'] == 'Ball_8'||$oddsResult[$i]['g_type'] == 'Ball_9') && $k != 'g_type' && $v != null)
			{
				$arrList[$i][$k] = setoddsxj($k, $v,NULL, $user, 2,$oddsResult[$i]['g_type']);
			}
		}
	}
